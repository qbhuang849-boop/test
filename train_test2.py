...
def main(args):
    ...
    # 定义损失函数和优化器
    from utils_triple_loss import TripleCombinedLoss
    criterion = TripleCombinedLoss(focal_weight=0.4, dice_weight=0.4, ce_weight=0.2,
    ...
    for name, param in model.named_parameters():
        if 'backbone' in name:
            backbone_params.append(param)  # 骨干网络：官方预训练，小学习率
        else:
            decoder_params.append(param)  # 解码器：随机初始化，大学习率
    optimizer = optim.Adam([
        {'params': backbone_params, 'lr': args.lr * 0.1},  # 骨干学习率：1e-5
        {'params': decoder_params, 'lr': args.lr}          # 解码器学习率：1e-4
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', type=str,
                        default=r'E:\工作文件\20260327_司风空拍模型数据_2262\data_train',
                        help='数据集根目录路径')
    ...
    parser.add_argument('--pretrained', type=str, default=r'C:\Users\yj\Desktop\git\Deeplabv3+\models\train1\best_model.pth',#train2
    ...
    parser.add_argument('--num_workers', type=int, default=4,
                        help='数据加载的工作进程数')
    parser.add_argument('--save_dir', type=str, default='./models/train6-1',
                        help='模型保存目录')
